...
- **Weekly Checkpoint write (at week boundary)** — **One location: `episodes/weekly/{YYYY-Www}_{label}.md`**, written by the weekly periodic close (`/close weekly`), with `week:` / `range:` / `written:` frontmatter. Week is Monday-anchored; `range:`'s end date is what staleness is measured against. Written at the first session of each new week (or sooner if a session naturally ends a week). **Two namespaces.** `weekly/` holds checkpoints written in their own week; `weekly/retrospective/` holds ones reconstructed later. Separate directories, not a `status:` field — a field is a soft control and eventually one consumer forgets to filter. Anything that needs contemporaneous evidence reads `weekly/*.md` and gets that guarantee from the path; coverage checks count both. A retrospective file also records what it was built from and how much of the source was read, and must never be cited as a record of the week it describes. Measured limit: a week whose own daily logs are themselves reconciliation backfills yields nothing independent — do not write a third layer, register the week as not backfillable and say why.

**Schema (v2, 2026-08-18): 10-20 lines, hard cap 20 lines / 2 KB.** One bullet per active project (state + what changed), then exactly one `Surprised:` line and one `Stuck:` line. **What earns a bullet.** Order by what the week was *about*, not by session count: research questions asked and answered, decisions and their reasons, results that changed direction, work with an outside deadline. Tooling — agent wiring, skill/flag audits, dashboards, config cleanup — is compressed into one bullet at the end or dropped, however many sessions it took. A week can be 60% tooling by session count and still be a research week by what mattered; the checkpoint records the latter. If tooling genuinely *was* the week, say so in `Surprised:` rather than spreading it across the bullets. No `Episodic Highlights`, `Semantic Patterns`, `Keep/Problem/Try`, or `Next Week Plan` — those are monthly-level and their presence is what split the format in two. The old "1-2 lines" target was never met: measured 2026-08-18, real files run 13-48 lines, so the spec was understating practice by 10-25x and the gap between them is where drift lived. `2026-W25` keeps its extended shape as a legacy file; it is not migrated and not a template. If the bootstrap's `maintain-recent.py` finds the newest checkpoint older than 7 days **or finds none at all**, it emits a directive under the bootstrap's `_RECENT.MD MAINTENANCE` heading; write the missing week's checkpoint before continuing. "No notable surprises / no stuck items" is a valid entry; presence matters more than depth. **Requirement, not description:** from `2026-W34` onward, monthly consolidation must read that month's checkpoints first and name them in the reflection's sources. This is written as a rule because the reverse was measured — the 2026-05/06/07 reflections cite zero checkpoints and the 2026-05 one names raw daily logs plus ledger reconciliation as its actual sources. Until the monthly step genuinely reads them, this rung is decorative and paying for it is waste. **Falsification:** if a monthly reflection built from checkpoints is not at least as good as one built from raw logs, at a fraction of the reading, retire the rung rather than keep funding it.
